...
@section('title', 'Validación de Ficha')

@section('content_header')
    <div class="header-flex">
        <h1>Revisión de Ficha</h1>
        <div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        ...
                        @foreach ($fichasDocumentos as $ficha)
                            @php
                                $estado = json_decode($ficha->estado, true) ?? [];
                            @endphp
                            @if ($ficha->file_path && (!isset($estado['estado']) || $estado['estado'] == 'rechazar'))
                                @php
                                    $documentosParaRevisar = true;
                                @endphp
                                <form class="update-form"
                                    data-url="{{ route('ValidarFicha.updateDocumento', ['id' => $usuario->id, 'fichaId' => $ficha->id]) }}"
                                    method="POST">
                                    @csrf
                                    @method('PUT')
                                    <div class="form-group row">
                                        <label
                                            class="col-sm-2 col-form-label">{{ ucfirst(str_replace('_', ' ', $ficha->nombre)) }}</label>
                                        <div class="col-sm-4">
                                        ...
                                        <div class="col-sm-4">
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="documento_estado"
                                                    id="validar_{{ $ficha->nombre }}" value="validar">
                                                <label class="form-check-label"
                                                    for="validar_{{ $ficha->nombre }}">Validar</label>
                                            </div>
                                            <div class="form-check form-check-inline">
                                                <input class="form-check-input" type="radio" name="documento_estado"
                                                    id="rechazar_{{ $ficha->nombre }}" value="rechazar">
                                                <label class="form-check-label"
                                                    for="rechazar_{{ $ficha->nombre }}">Rechazar</label>
                                            </div>
