<!-- Modal -->
<div class="modal fade" id="uploadCedulaModal" tabindex="-1" role="dialog" aria-labelledby="uploadCedulaModalLabel"
    aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content border-success shadow-sm">
            <div class="modal-header bg-success text-white">
                <h5 class="modal-title" id="uploadCedulaModalLabel">Subir Cédula de Evaluación</h5>
                <button type="button" class="close text-white" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <!-- Formulario para subir la cédula de evaluación -->
                <form action="{{ route('cedula.store') }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    ...
                    <div class="form-group mt-2">
                        ...
                        <input type="text" class="form-control" id="nombre" name="nombre"
                            value="Cedula_Evaluación_{{ $estandar->name }}_{{ auth()->user()->matricula }}_{{ auth()->user()->name }}_{{ auth()->user()->secondName }}_{{ auth()->user()->paternalSurname }}_{{ auth()->user()->maternalSurname }}"
                            readonly>
                        <small class="form-text text-muted">Asegúrate de firmar tu Cédula de Evaluación.</small>
                        @error('nombre')
                    ...
                    <button type="submit" class="btn btn-success mt-3">Subir Cédula de Evaluación</button>
                </form>
            </div>
        </div>
    </div>
