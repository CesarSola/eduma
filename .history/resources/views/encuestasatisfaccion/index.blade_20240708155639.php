...
@section('content')
    ...
    <div class="mt-4">
        <!-- Botón para descargar formulario lleno -->
        <a href="/form/download" class="btn btn-success">Descargar Formulario Lleno</a>

        <!-- Botón para descargar formulario vacío -->
        <a href="/form/download-empty" class="btn btn-info">Descargar Formulario Vacío</a>
    </div>

    <script>
        function setEmoji(questionNumber, emoji) {
            // Remove 'selected' class from all emojis for the current question
            document.querySelectorAll('#question' + questionNumber + ' ~ .emoji').forEach(function(el) {
