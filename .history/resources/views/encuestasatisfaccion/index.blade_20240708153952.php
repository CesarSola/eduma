    ...
    <form action="/form" method="POST">
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                ...
                <tr>
                    <td>{{ $question }}</td>
                    <td><span class="emoji {{ old('question' . ($index + 1)) == '😀' ? 'selected' : '' }}" onclick="setEmoji({{ $index + 1 }}, '😀')">😀</span></td>
                    <td><span class="emoji {{ old('question' . ($index + 1)) == '🙂' ? 'selected' : '' }}" onclick="setEmoji({{ $index + 1 }}, '🙂')">🙂</span></td>
                    <td><span class="emoji {{ old('question' . ($index + 1)) == '😐' ? 'selected' : '' }}" onclick="setEmoji({{ $index + 1 }}, '😐')">😐</span></td>
                    <td><span class="emoji {{ old('question' . ($index + 1)) == '☹️' ? 'selected' : '' }}" onclick="setEmoji({{ $index + 1 }}, '☹️')">☹️</span></td>
