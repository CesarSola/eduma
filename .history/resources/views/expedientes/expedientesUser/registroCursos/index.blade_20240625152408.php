...
@section('content')
    <div class="col-md-12 mb-8">
        <div class="card h-100">
            ...
            <div class="card-body">
                @if ($cursos->isEmpty())
                    <h6 class="text-center">Por el momento no hay ningun curso disponible.</h6>
                @else
                    @php
                        $usuario = Auth::user();
                        $inscrito = false;
                    ...
                    @foreach ($cursos as $curso)
                        @php
                            $comprobanteExistente = $usuario
                                ->comprobantes()
                                ->where('estandar_id', $curso->id)
                                ->first();

                            if ($comprobanteExistente) {
                                $inscrito = true;
                                $competenciaInscrita = $curso;
                            }
                        @endphp
                        <div class="col-md-6 mb-4">
                            <div class="card h-100">
                                <div class="card-body d-flex justify-content-between align-items-center">
                                    ...
                                    @else
                                        <a href="{{ route('competenciaEC.show', ['competenciaEC' => $                                            <h6 class="text-center">Mis cursos</h6>
->id]) }}"
                                            class="btn btn-primary">Inscribirse</a>
                                    @endif
                                    @if ($inscrito && $curso->id === $competenciaInscrita->id)
                                        <div class="card-footer">
                                            <h6 class="text-center">Ve a la pestaña de</h6>
                                            <h6 class="text-center">Mis cursos</h6>
                                        </div>
                                    @endif
                                </div>
                            </div>
