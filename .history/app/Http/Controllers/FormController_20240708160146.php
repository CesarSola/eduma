...
class FormController extends Controller
{
    ...
    public function submitForm(Request $request)
    {
        ...
        foreach (range(1, 8) as $index) {
            ...
            // Agregar la carita seleccionada con un estilo específico (ejemplo: color de fondo)
            switch ($selectedEmoji) {
                case '😀':
                    $section->addText('😀', array('color' => '00FF00')); // Verde para Muy de acuerdo
                    break;
                case '🙂':
                    $section->addText('🙂', array('color' => 'FFFF00')); // Amarillo para De acuerdo
                    break;
                case '😐':
                    $section->addText('😐', array('color' => 'FFA500')); // Naranja para Parcialmente en desacuerdo
                    break;
                case '☹️':
                    $section->addText('☹️', array('color' => 'FF0000')); // Rojo para Totalmente en desacuerdo
                    break;
                default:
                    $section->addText(''); // Sin respuesta
                    break;
            }
